	...
	config = function()
		...
		local mappings = {
			n = {
				...
				["<leader>c"] = {
					":lua require('cokeline.handlers')['close_'.. vim.api.nvim_get_current_buf()](_,_,'l')<CR>",
		...
		for i = 1, 9 do
			set("n", ("<F%s>"):format(i), ("<Plug>(cokeline-focus-%s)"):format(i), {
				desc = ("Focus buffer %s"):format(i),
				silent = true,
			})

			set("n", ("<leader>%s"):format(i), ("<Plug>(cokeline-focus-%s)"):format(i), {
